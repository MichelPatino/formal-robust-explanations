...
#chmod u+x abduction_algorithm/new_algorithm_weighted.sh
#execute:
#./abduction_algorithm/new_algorithm_weighted.sh
...
# for now, it is the number of variables
...
do
    ...
    if [ $n -eq $highest_input_variable_subscript ]; then
        ...
        #Make the explanation all zeros because is not was not a roboust explanation
        ...
        #It is a robust explanation for a weighted epsilon.      
        # check if it is a cardinality minimal explanation
        echo "determining if the explanation is cardinality minimal"
        #start with the variable of lowest index
        variable_index=0
            ...
            echo "the line read in the explanation file is: "$line
                if [ "$line" = "1" ]; then
                    ...
                    echo "perturbing explanation constraint "$variable_index" with weighted epsilon"
                    ...
                    if [ "$result" = "unsat" ] || [ "$result" = "timeout" ]; then
                        echo "the explanation was not minimal because the verifier outputs: "$result
                        python $python_files_folder/all_zeros_to_csv.py $explanation_file_address $instances_folder/$instance_name $instance_name $epsilon $number_of_variables $results_folder $name_of_benchmark
                        break
                    else
                        echo "restore constraint: "$variable_index
                        ...
                        ##############################################################################
                        if [ $variable_index -eq $highest_input_variable_subscript ]; then
                    ...
                    echo "skiping variable "$variable_index" outside the explanation"
                    if [ $variable_index -eq $highest_input_variable_subscript ]; then
                        
                        echo "Empty explanation, initial perturbation may be too weak!"
                            python $python_files_folder/all_zeros_to_csv.py $explanation_file_address $instances_folder/$instance_name $instance_name $epsilon $number_of_variables $results_folder $name_of_benchmark             
            ...
            echo "loop number: "$variable_index
